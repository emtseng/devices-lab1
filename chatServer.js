...
io.on('connect', function (socket) {
  ...
  socket.on('loaded', function () {// we wait until the client has loaded and contacted us that it is ready to go.
    ...
    socket.emit('answer', "Helloooo! I am \"___*-\", a simple chatbot example."); //We start with the introduction;
    setTimeout(timedQuestion, 2500, socket, "What's your name?"); // Wait a moment and respond with a question.
...
function bot(data, socket, questionNum) {
  ...
  switch (questionNum) {
    ...
    case 1:
      ...
      waitTime = 5000;
      question = 'What\'s your mother\'s maiden name?';			    	// load next question
      break
    case 2:
      answer = 'That\'s cool. I\'m a robot, so I don\'t think I have a mom. But yours sounds nice.';
      waitTime = 5000;
      question = 'How about your first pet? What were they called?';			    	// load next question
      break
    case 3:
      answer = `I always wanted a pet. But I'm a robot, so. ${input} must have been a great pet.`;
      // socket.emit('changeBG', input.toLowerCase());
      waitTime = 5000;
      question = `I'm so curious about your life now. What was the name of the street that you grew up on?`;			    	// load next question
      break
    case 4:
